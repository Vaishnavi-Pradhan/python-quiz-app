...
def create_window(name,phone):
    ...
    def check_answers():
        for ans in selected_answers:
            i = ans.get()                  #By get function we will get the actual selectec value of radio buttons
            answers.append(i)
        ...
        root.destroy()
        result_page.create_window(name,phone,"Python",points)
    ...
    label=tk.Label(inner_frame,text=" Python Test ",wraplength=300,bg="pink", font=("Arial", 22), borderwidth=2, relief="solid", height=2)
    label.pack(padx=250,pady=30, anchor="n")
    
    for i,v in enumerate(ran):  # 10,20,30,11,24,......
        query = "SELECT ques FROM py_questions WHERE id=%s"
        ...
        query = "SELECT op1 FROM py_questions WHERE id=%s"
        cursor.execute(query,(v,))
        ...
        query = "SELECT op2 FROM py_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT op3 FROM py_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT op4 FROM py_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT ans FROM py_questions WHERE id=%s"
        cursor.execute(query,(v,))
        ans=cursor.fetchone()
        db_ans.append(ans[0])  
