...
def create_window(name,phone):
    ...
    def check_answers():
        ...
        root.destroy()
        result_page.create_window(name,phone,"C/C++",points)
    ...
    label=tk.Label(inner_frame,text=" C / C++ Test ",wraplength=300,bg="pink", font=("Arial", 22), borderwidth=2, relief="solid", height=2)
    label.pack(padx=250,pady=30, anchor="n")
    
    for i,v in enumerate(ran):
        query = "SELECT ques FROM c_questions WHERE id=%s"
        ...
        query = "SELECT op1 FROM c_questions WHERE id=%s"
        cursor.execute(query,(v,))
        ...
        query = "SELECT op2 FROM c_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT op3 FROM c_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT op4 FROM c_questions WHERE id=%s"
        cursor.execute(query,(v,))
        option=cursor.fetchone()
        op=option[0]
        ...
        query = "SELECT ans FROM c_questions WHERE id=%s"
        cursor.execute(query,(v,))
        ans=cursor.fetchone()
        db_ans.append(ans[0])
